...
const grade_antiga = {
  "Laboratório de Informática": {
    ch: 71,
    eq: grade_nova["Desenvolvimento Sem Código"],
  },
  Algoritmos: { ch: 71, eq: grade_nova["Fundamentos de Programação"] },
  "Fundamentos de Matemática": {
    ch: 71,
    eq: grade_nova["Lógica e Introdução à Estatística"],
  },
  "Organização de Computadores": { ch: 35 },
  "Introdução a Análise e Desenvolvimento de Sistemas": {
    ch: 35,
    eq: grade_nova["Introdução a Análise e Desenvolvimento de Sistemas"],
  },
  "Comunicação Oral e Escrita": {
    ch: 71,
    eq: grade_nova["Comunicação Oral e Escrita"],
  },
  "Programação Orientada a Objetos": {
    ch: 71,
    eq: grade_nova["Programação Orientada a Objetos"],
  },
  ...
  "Técnicas de Programação": {
    ch: 71,
    eq: grade_nova["Estratégias de Programação"],
  },
  "Programação Desktop": {
    ch: 71,
    eq: grade_nova["Programação Web Front-end"],
  },
  "Estrutura de Dados": { ch: 71, eq: grade_nova["Estrutura de Dados"] },
  "Banco de Dados 2": { ch: 71, eq: grade_nova["Banco de Dados 2"] },
  "Sistemas Operacionais": {
    ch: 71,
    eq: grade_nova["Introdução aos Sistemas Operacionais"],
  },
  "Análise e Projeto Orientado a Objetos": {
    ch: 71,
    eq: grade_nova["Análise e Projeto Orientado a Objetos"],
  },
  "Programação Web": { ch: 71, eq: grade_nova["Programação Web Back-end"] },
  "Gerenciamento de Projetos de Software": {
    ch: 71,
    eq: grade_nova["Gerenciamento de Projeto de Software"],
  },
  ...
  "Oficina de Integração": {
    ch: 68,
    eq: grade_nova["Disciplina Certificadora Específica"],
  },
  "Metodologia de Pesquisa": { ch: 35 },
  "Programação Móvel": {
    ch: 71,
    eq: grade_nova["Programação para Dispositivos Móveis"],
  },
  "Arquitetura de Software": {
    ch: 71,
    eq: grade_nova["Arquitetura de Software"],
  },
  "Interação Homem Computador": {
    ch: 71,
    eq: grade_nova["Interação Homem Computador"],
  },
  "Programação Distribuída": {
    ch: 71,
    eq: grade_nova["Programação Distribuída"],
  },
  "Segurança e Auditoria de Sistemas": {
    ch: 71,
    eq: grade_nova["Segurança e Auditoria de Sistemas"],
  },
  "Qualidade de Software": { ch: 35 },
  "Tópicos em Computação": {
    ch: 71,
    eq: grade_nova["Programação Web Fullstack"],
  },
  "Gestão Financeira": { ch: 35, op: true },
  Economia: { ch: 35, eq: grade_nova["Economia"], op: true },
  "Gestão Empresarial": {
    ch: 35,
    eq: grade_nova["Estratégias de Inovação"],
    op: true,
  },
  ...
  "Estágio Curricular Obrigatório": {
    ch: 400,
    r: true,
    eq: grade_nova["Estágio Curricular Obrigatório"],
  },
};
...
const render_sum = (grade, target, opts) => {
  ...
  for (let unidade in grade) {
    ...
    if (dados.op) {
      if (dados.realizado) ops_count++;
    } else {
      if (!dados.realizado) result += dados.ch;
    }
  }
  if (ops_count < 3) opts.classList.remove("OK");
  else opts.classList.add("OK");
  result += 180 - Math.min(180, ops_count * 60);
  target.innerHTML = `${result}h`;
};
const render_grade = (grade, target) => {
  target.innerHTML = "";
  for ( const unidade in grade) {
    const li = document.createElement("li"),
      dados = grade[unidade];
    li.innerHTML = `${unidade} - ${dados.ch}h ${dados.r ? "(REMOTA)" : ""} ${
      dados.op ? "(OPTATIVA)" : ""
    }`;
    if (dados.realizado) li.classList.add("realizado");
    if (dados.op) li.classList.add("optativa");
    (() => {
      const cur = unidade;
      li.addEventListener("click", function () {
        ...
        render_sum(
          ...
          target.parentElement.querySelector(".optativas span")
        );
      });
    })();
